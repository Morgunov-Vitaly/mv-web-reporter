...
/* !!!!!!!!!!!!!!! Конструктор блока вывода переменных по шорткоду !!!!!!!!!!!!!!!! */
		...
		<script  type="text/javascript">
			...
			jQuery( function( $ ){
				...
				$("#form_login").submit(function( event ){ /* Отправляем данные формы  */
					$("body").addClass("mv-cursor-whait"); // устанавливаем курсор ожидание почему-то не видно
					$.ajax({
						type: 'GET',
			...
			jQuery( function( $ ){
				...
				$("#form_param").on( "submit", function( event ) {  /* отправка данных формы с параметрами для построения отчета */
					$("body").addClass("mv-cursor-whait"); // устанавливаем курсор ожидание
					$.ajax({
						type: 'GET',
