<?php
	/*
		...
		Обработчик и Конструктор таблицы для 102t отчета и его вспомогательные функции 
	...
	*/
	...
	function mv_take_report_data_102t() {
		...
		// Если ответ сервера 200 OK, то можно вывести что-нибудь
		if ( ! is_wp_error( $mv_remote_get )  &&  wp_remote_retrieve_response_code( $mv_remote_get ) == 200 )  {
			...
			$mv_html = mv_102t_constructor($sum_bonusAddTotal, $sum_bonusPayTotal, $sum_bonusSalesAmount, $sum_cardSalesAmount, $sum_cardSalesCount, $sum_ordersCount, $sum_ordersWithCardCount, $sum_revenueFactSum, $sum_revenuePlanSum, $sum_salesRevenue, $sum_salesTotal); //вызваем конструктор аккордеона
			//PC::debug( $mv_html );
