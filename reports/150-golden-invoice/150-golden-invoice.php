...
/*
	...
	Менеджер отчета Golden Invoice
	ID 150 
...
add_action( 'wp_footer', 'enqueue_mv_stylecss_150' );
/* Подвешиваем к хуку функцию подключения стилей */	

function enqueue_mv_stylecss_150() {
	/* Проверяем наличие шорткода  в посте */
	global $mv_report_params;	
	//PC::debug($mv_report_params['id']);
	if ($mv_report_params['id'] == 150) {
		wp_register_style( 'mv_stylecss_150', plugins_url('css/report-150.css', __FILE__));
		wp_enqueue_style( 'mv_stylecss_150' );
	}
}
/* / Подключаем стили !!!!!!!!!! */	

/* !!!!!!! Подключаем AJAX обработчик отчета 150 JS !!!!!!!! */
require_once( plugin_dir_path( __FILE__ ) . 'handlers/report-constructor-150.php' );
require_once( plugin_dir_path( __FILE__ ) . 'handlers/report-handler-150.php' );


function mv_150_report(){
	global $post;
	...
	jQuery("#form_param").submit(function (event_pr) { /* отправка данных формы с параметрами для построения отчета */
		if (mv_document_ready > 0) {		
			...
			$.ajax({
				...
				data: {
					action: 'mv_take_report_data_150', /* Вызывам обработчик делающий запрос данных отчета*/
					mv_nonce: '<?php echo wp_create_nonce( "mv_take_report_data_150" ); ?>',
