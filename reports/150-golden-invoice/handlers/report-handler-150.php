<?php
	/*
		...
		PHP обработчик запроса на удаленный сервер данных 150 отчета 
		
	*/
	
	
	add_action('wp_ajax_mv_take_report_data_150' , 'mv_take_report_data_150'); /* Вешаем обработчик mv_take_report_data на ajax  хук */
	add_action('wp_ajax_nopriv_mv_take_report_data_150', 'mv_take_report_data_150'); /* то же для незарегистрированных пользователей */
	
	function mv_take_report_data_150() {
		
		$nonce = $_GET['mv_nonce']; // Вытаскиваем из AJAX запроса переданное значение mv_nonce и заносим в переменную $nonce
		// проверяем nonce код, если проверка не пройдена прерываем обработку
		if( ! wp_verify_nonce( $nonce, 'mv_take_report_data_150' ) ) wp_die('Stop! Nonce code of mv_take_report_data_150 incorrect!');
		...
		/* https://cscl.coffeeset.ru/ws-test/web/report?token=...&id=150&dateFrom=...&dateTo=...&refDivision=193A9F3B-15AE-4030-B3BA-6DE2DA537383
		*/
		$mv_url = 'https://cscl.coffeeset.ru/ws-test/web/report?token=' . $token . '&id=150&dateFrom=' . $dateFrom . '&dateTo='  . $dateTo . '&refDivision=' . $refObject; // Формируем строку запроса
		//$mv_url = "https://cscl.coffeeset.ru/ws-test/web/report?token=YTY0OTYxY2UtYTgwNS00N2M3LTg1YzctZjMyNTU3YTUyMTFj&id=150&dateFrom=2017-07-17T00:00:01&dateTo=2017-07-17T23:59:59&refDivision=6A003DBF-EA0C-4ECE-BE9E-2C43A79434A3";
		PC::debug($mv_url );	
		...
		if ( ! is_wp_error( $mv_remote_get )  &&  wp_remote_retrieve_response_code( $mv_remote_get ) == 200 )  {
			...
			*/
			 if(! empty( $mv_report_result->employeeSummary ) ){
				$mv_html = mv_150_report_constructor($mv_report_result); 
				} else {
	...
	/* !!!!!!!!!!! / PHP обработчик AJAX запроса данных 150 отчета !!!!!!!!!!! */
?>