...
/*
	...
	Менеджер отчета Bonuses Бонусы
	ID 120 
...
add_action( 'wp_footer', 'enqueue_mv_stylecss_120' );
/* Подвешиваем к хуку функцию подключения стилей */	

function enqueue_mv_stylecss_120() {
	/* Проверяем наличие шорткода  в посте */
	global $mv_report_params;	
	//PC::debug($mv_report_params['id']);
	if ($mv_report_params['id'] == '120') {
		wp_register_style( 'mv_stylecss_120', plugins_url('css/report-120.css', __FILE__));
		wp_enqueue_style( 'mv_stylecss_120' );
	}
}
/* / Подключаем стили !!!!!!!!!! */	
...
function enqueue_mv_120_jquery() {
	/* Проверяем наличие шорткода в посте */
	global $mv_report_params;
	if ($mv_report_params['id'] == '120') {
		
		wp_register_script( 'mv-120-script-js', plugins_url('js/mv-120-script.js', __FILE__), array( 'jquery' ), '1.0', true);
		wp_enqueue_script( 'mv-120-script-js' ); /* регистрирую скрипт обработчика */
		
		$mv_dataToBePassed = array(
		'mv_admin_url' => admin_url( "admin-ajax.php" ), /* Путь до admin-ajax.php */
		'mv_nonce' => wp_create_nonce( "mv_take_report_data_120" ), /* проверочное значение nonce */
		...
		wp_localize_script( 'mv-120-script-js', 'mv_php_vars', $mv_dataToBePassed );
...
add_action( 'wp_footer', 'enqueue_mv_120_jquery' );
/* / Подключаем скрипты */



/* !!!!!!! Подключаем AJAX обработчик отчета 120 JS !!!!!!!! */
require_once( plugin_dir_path( __FILE__ ) . 'handlers/report-constructor-120.php' );
require_once( plugin_dir_path( __FILE__ ) . 'handlers/report-handler-120.php' );


function mv_120_report(){
	ob_start();
	?>
